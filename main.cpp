...
void save()
{
  ...
  saveFile <<"Resistance:" <<"," << R <<endl;
  saveFile <<"Capacitance:" <<"," << C << endl;
  saveFile <<"Simulation Time:" <<"," <<t <<endl;
  saveFile <<" " <<"," <<"Amplitude" <<"," <<"Frequency" <<",";
  saveFile <<"Phase Shift (Rads)" <<"," <<"DC Off Set" <<"," <<"Wave Form";
  saveFile << endl;
  for (int i=0; i<5; i++)
  {
    saveFile <<"Wave " <<i+1 <<"," << inputs[i].amp << "," <<inputs[i].freq;
    saveFile <<"," <<inputs[i].phase <<"," <<inputs[i].dc <<",";
    if (inputs[i].form == 'a')
    {
      saveFile << "Sinusoid" <<endl;
    }
    else if (inputs[i].form == 'b')
    {
      saveFile << "Square" <<endl;
    }
    else if (inputs[i].form == 'c')
    {
      saveFile << "Triangle" <<endl;
    }
    else
    {
...
int main(int argc, char *argv[])
{
    ...
    chargeVect.push_back(0); //initial charge of capacitor is set to 0
    runge();
